...
- Python 3.11+
- `.env` for API key
- `note_refinery.yaml` for provider, models, patch mode, and timeout
...
4. `run`
   - runs `review -> patch -> verify` in one command
...
```powershell
py -3 -m note_refinery_simple run --notes-dir "C:\Users\HOANG PHI LONG DANG\MinerU" --output-root ".\live-run" --provider custom --base-url "https://my-gateway.example.com/v1" --patch-model deepseek-v4-pro
```
